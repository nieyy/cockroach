...
func httpPost(c postContext, request, response gogoproto.Message, method fmt.Stringer) error {
	...
	for r := retry.Start(c.RetryOpts); r.Next(); {
		...
		req.Header.Add(ContentTypeHeader, ProtoContentType)
		req.Header.Add(AcceptHeader, ProtoContentType)
		req.Header.Add(AcceptEncodingHeader, SnappyEncoding)

		resp, err = client.Do(req)
		if err != nil {
			// TODO(pmattis): Provide an interface for overriding logging.
			// if log.V(1) {
			// 	log.Warning(err)
			// }
			continue
		...
		if resp.Header.Get(ContentEncodingHeader) == SnappyEncoding {
